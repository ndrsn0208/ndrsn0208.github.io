...
test('the homepage shows direct contact links and news without a separate Contact destination', async ({ page }) => {
  ...
  await expect(navigation(page).locator('[data-nav-destination]')).toHaveCount(4)
  await expect(destination(page, 'blog')).toHaveAttribute('href', '/blog')
  ...
  await expect(news.getByRole('link', { name: 'Self-Consolidating Language Models', exact: true })).toHaveAttribute('href', '/blog/self-consolidating-language-models')
